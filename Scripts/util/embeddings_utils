...
def get_embedding(text: str, model=config.EMBEDDING_MODEL, **kwargs) -> List[float]:
...
async def aget_embedding(
    text: str, model=config.EMBEDDING_MODEL, **kwargs
) -> List[float]:
    # replace newlines, which can negatively affect performance.
    text = text.replace("\n", " ")
...
def get_embeddings(
    list_of_text: List[str], model=config.EMBEDDING_MODEL, **kwargs
) -> List[List[float]]:
